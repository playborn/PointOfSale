...
for /F "usebackq tokens=2,3 delims=;" %%a in (`call lib\warenkorb printCSVsingle %1 ";"`) do (
	set "betrag=%%~a"
	set "name=%%~b"
	echo.!betrag!;"!name!";"EINZAHLUNG"|call lib\accounts doTransaction %2 "!customerID!"
	if /I not "!customerID!"=="!kassenID!" echo.!betrag!;"!name!";"EINZAHLUNG"|call lib\accounts doTransaction %2 "!kassenID!"
)
exit /b 0

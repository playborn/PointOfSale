...
for /F "usebackq delims=" %%i in (`call lib\accounts iterAccounts %1`) do (
	echo. # rufe informationen ab, Konto [ %%~i ]
	call lib\accounts accountGetCacheBalance %1 "%%~i" guthaben
	if "%%i"=="%kassenID%" (
...
>"%file%" call lib\accounts exportTransactionsToCSV %1
rem >"%file%" call lib\kunden exportTransactionsToCSV %1
exit /b 0

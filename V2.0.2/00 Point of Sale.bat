...
call lib\accounts accountExist kunden "%kassenID%" || (
	call lib\accounts accountCreate kunden "%kassenID%" || call :error_FATAL " ACCOUNT konnte nicht ersellt werden"
)
:main
call :verkauf
goto main
...
call :colorNormal
if "%customerID%"=="UNLOCK" cls & call :hidden_forceUnlock kunden
call :kundeScannen kunden customerID || (call :colorNotFound & echo. # Kundennummer ist nicht registriert #& goto verkauf)
:verkauf_changeCustomer
call :colorSuccess
call :getCustomerInfos kunden "%customerID%"
...
if /I "%input%"=="V" (
	...
	call lib\accounts requestNewTransactionID kunden transactionID
	echo. #	Transaktions ID: [ !transactionID! ]
	start /min cmd /c lib\workerVerkauf warenkorb kunden "%customerID%"
	echo [ Erfolgreich ausgeführt ]
	rem call :warenkorb.abziehen warenkorb kunden "%customerID%"
	exit /b 0
)
if /I "%input%"=="S" (
	call lib\warenkorb isEmpty warenkorb && goto verkauf_continue
	...
	call lib\accounts requestNewTransactionID kunden transactionID
	echo. #	Transaktions ID: [ !transactionID! ]
	start /min cmd /c lib\workerStorno warenkorb kunden "%customerID%"
	echo [ Erfolgreich ausgeführt ]
	rem call :Warenkorb.gutschreiben warenkorb kunden "%customerID%"
	exit /b 0
)
if /I "%input%"=="E" (
	call lib\warenkorb isEmpty warenkorb && goto verkauf_continue
	...
	call lib\accounts requestNewTransactionID kunden transactionID
	echo. #	Transaktions ID: [ !transactionID! ]
	start /min cmd /c lib\workerEinzahlen warenkorb kunden "%customerID%" "%kassenID%"
	echo [ Erfolgreich ausgeführt ]
	rem call :warenkorb.einzahlen warenkorb kunden "%customerID%" "%kassenID%"
	exit /b 0
)
if /I "%input%"=="A" (
	call lib\warenkorb isEmpty warenkorb && goto verkauf_continue
	...
	call lib\accounts requestNewTransactionID kunden transactionID
	echo. #	Transaktions ID: [ !transactionID! ]
	start /min cmd /c lib\workerAuszahlen warenkorb kunden "%customerID%" "%kassenID%"
	echo [ Erfolgreich ausgeführt ]
	rem call :warenkorb.auszahlen warenkorb kunden "%customerID%" "%kassenID%"
	exit /b 0
)
if /I "%input%"=="K" (
	rem kunde wechseln
...
if /I "%input%"=="T" (
	...
	call :colorSuccess
	pause
	rem kundenguthaben könnte sich geändert haben 
	call :getCustomerInfosFast kunden "%customerID%"
	goto verkauf_continue
)
IF "%input%"=="UNLOCK" (
	echo ### ADMIN FUNKTION ###
	...
	pause
	call lib\accounts accountForceUnlock kunden "%customerID%"
	rem call lib\kunden customerForceUnlock kunden
	call :colorSuccess
	goto verkauf_continue
...
call lib\accounts forceUnlockAll %1
...
:kundeScannen kundenObj returnVar
setlocal
set "customerID="
...
endlocal & (
	...
	call lib\accounts accountExist %1 "%customerID%" || exit /b 1
)
exit /b 0
...
if defined betrag (
	...
	rem prevent 32-bit overflow, verhindert aber auch lange rechnungen
...
:getCustomerInfos kundenObj "customerID"
rem returns customerID customerBalance customerIDstr customerBalanceStr customerStr
setlocal
echo.
...
echo.!betrag!;"!name!";"!transactionCode!"|call lib\accounts doTransaction %1 "%customerID%"
rem call lib\kunden customerSelect %1 "%customerID%"
rem call lib\kunden customerDoTransaction %1 "%betrag%" "%name%" "%transactionCode%"
...
for /F "usebackq tokens=1-6 delims=;" %%a in (`call lib\accounts exportTransactionsByTransactionID %1 "%~3"`) do (
...
echo.
call lib\accounts requestNewTransactionID %1 transactionID
rem call lib\kunden requestNewTransactionID %1 transactionID
